import { IAdminUser, IAdminUsersContentItem } from "../../../types/Admin"
import transformPhoto from "../../../utils/transformPhoto"

const AdminUsersContentItem = ({ user, setUsersSelected }: IAdminUsersContentItem) => {
   const handleCheckboxChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      setUsersSelected((prevSelected: IAdminUser[]) =>
         event.target.checked
            ? [...prevSelected, user]  // Добавляем ID, если чекбокс включен
            : prevSelected.filter(item => item.id !== user.id) // Удаляем ID, если чекбокс выключен
      );
   };
   ...
   return (
      <div className="admin-users__content">
         <div className="admin-users-item">
            <div className="admin-users-item__right">
            ...
            <p className="admin-users-item__login">{user.login}</p>
            <p className="admin-users-item__mail">{user.mail}</p>
            <p className="admin-users-item__create">{user.date_of_create}</p>
            <p className="admin-users-item__role">{user.role ? user.role : "Пользователь"}</p>
         </div>
      </div>
   )
}

export default AdminUsersContentItem