...
const CommentsContentItem = ({ id, account_id, parent_id, text, date_comment, author_nickname, quoted_author_nickname, quoted_comment_text, account_name, setResult }: ICommentItem) => {
   ...
   const editComment = async () => {
      try {
         ...
         setSuccess('Коментарий успешно отредактирован')
         setIsOpenEdit(false)
         setResult((prev: ICommentsState) => ({
            ...prev,
            items: prev.items ? prev.items.map((commentItem) => commentItem.id == id ? { ...commentItem, text: comment } : commentItem) : prev.items
         }))
      } catch (error) {
         setError('Произошла ошибка при редактировании коментария')
      }
   }
   ...
   const removeComment = async () => {
      try {
         ...
         setSuccess('Коментарий успешно удален')
         setIsOpenEdit(false)
         setResult((prev: ICommentsState) => ({
            ...prev,
         ...
         setError('Произошла ошибка при удалении коментария')
      }
   }
      ...
      <div className="comment">
         ...
         <div className="comment-header">
            <p className="comment-header__title">
               <span>{account_name}</span> <svg onClick={() => { navigate(`/${account_id}`) }} width="20" height="20" viewBox="0 0 70 70" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className="comment-header__button">
               <EditCommentModal editComment={editComment} removeComment={removeComment} setComment={setComment} comment={comment} isOpen={isOpenEdit} setIsOpen={setIsOpenEdit}>
                  <svg style={{ margin: '0 0 0 auto', cursor: 'pointer' }} width="20" height="20" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
         ...
         <div className="comment-text">{text.length > 100 ? text.slice(0, 100) + "..." : text}</div>
