...
const AccountCommenItem = ({ id, user_id, text, time_comment, author_nickname, children, onAction, setResult }: ICommentProps) => {
   ...
   const date = new Date
   const apiUrlRemove: string = 'http://167.86.84.197/api/delete-comment';
   const apiUrlAdd: string = 'http://167.86.84.197/api/add-reports';
   ...
   const removeComment = async (id: number) => {
      try {
         ...
         setResult((prev: IAccountState) => ({
            ...
            items: {
               ...prev.items,
               comments: prev.items ? removeNestedComment(prev.items.comments) : []
            }
         }));
      } catch (error) {
   ...
   const reportComment = async (id: number, reported_user_id: number, text: string,) => {
   ...
   const actionHandler = (action: string) => {
      switch (action) {
         ...
         case 'Удалить':
            ...
            break;
         case 'Изменить':
            onAction({ author_nickname, text, parent_id: id, }, 'edit')
            break;
   ...
   return (
      <>
         <div className="comment__item" >
            <div>
               {removeErrorMessage && <p style={{ color: 'red' }}>Что-то пошло не так, попробуйте ещё раз</p>}
            ...
            <div>
               {
                  user && user.id == user_id ? (
                     <div className="dropdown">
                        <div onClick={() => { setDropdownEdit(prev => !prev) }}>
                           {dropdownEdit ?
                              <svg style={{ margin: '3px 3px 0 auto', cursor: 'pointer' }} width="20" height="20" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                              ...
                              : <svg style={{ margin: '0 0 0 auto', cursor: 'pointer' }} width="25" height="25" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        {dropdownEdit && <div>
                           <p onClick={() => { actionHandler('Изменить') }}>Изменить <svg width="20" height="20" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                           ...
                           <p onClick={() => { actionHandler('Удалить') }}>Удалить <svg width="20" height="20" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                     ...
                     <div className="dropdown">
                        ...
                        {dropdownReport && <div>
                           ...
                           <p onClick={() => { actionHandler('Спам') }}>Спам <svg width="20" height="20" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
