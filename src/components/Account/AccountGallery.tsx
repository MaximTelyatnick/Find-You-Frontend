         ...
         <div className="account-gallery" style={{ padding: '40px 0 0 0' }}>
            {files.sort((a, b) => Number(a.endsWith(".mp4")) - Number(b.endsWith(".mp4"))).map((item) => (
               <div key={item} className="account-gallery__item" onClick={() => openModal(item)}>
                  {item.includes('.jpg') ? (
                     <img src={`http://167.86.84.197/api${item}`} alt="Image" />
                  ) : (
                     <video src={`http://167.86.84.197/api${item}`} autoPlay muted />
                  )}
               </div>
            ))}
         </div>
         ...
         {isOpen && activeItem && (
            <GalleryModal isOpen={isOpen} setIsOpen={setIsOpen}>
               {activeItem.includes('.jpg') ? (
                  <img src={`http://167.86.84.197/api${activeItem}`} alt="Image" />
               ) : (
                  <video src={`http://167.86.84.197/api${activeItem}`} autoPlay muted />
               )}
            </GalleryModal>
         )}
      </div>
