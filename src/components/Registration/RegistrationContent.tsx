...
const RegistrationContent = () => {
   ...
   const checkLogin = async () => {
      fetchData('get', `http://167.86.84.197/api/check-login/${login}`, setLoginAvailable)
   };
   ...
   const sendFormHandler = async (e: React.FormEvent<HTMLFormElement>) => {
      ...
      try {
         const response = await axios.post("http://167.86.84.197/api/register", { login, password, email, code });
